...
def public_job(job: dict) -> dict:
    ...
    keys = [
        ...
        "output_size_bytes", "trim_mode", "trim_applied",
...
def probe(path: Path) -> dict:
    # Native Render service uses imageio-ffmpeg, which bundles ffmpeg but not ffprobe.
    # FFmpeg itself exposes Duration + stream metadata in stderr, so parse that reliably.
...
def process_job(job_id: str):
    ...
    try:
        set_job(job_id, status="processing", progress=8, message="Inspecting source duration")
        src = probe(source)
        ...
        # Encoder delay/padding or odd containers can make the final a little longer.
        # Enforce a second pass if needed; never let an over-limit file escape.
