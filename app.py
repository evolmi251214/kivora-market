...
TARGET_FINAL_SECONDS = 418.0  # 6:58 target
HARD_FINAL_SECONDS = 419.0    # 6:59 hard pre-Roblox ceiling
...
POT_PROVIDER_URL = (os.getenv("POT_PROVIDER_URL") or "https://faz-dj-pot-provider.onrender.com").strip().rstrip("/")
YOUTUBE_COOKIES_B64 = (os.getenv("YOUTUBE_COOKIES_B64") or "").strip()
YOUTUBE_COOKIES_RAW = os.getenv("YOUTUBE_COOKIES_RAW") or ""
...
def prepare_cookie_file() -> Optional[str]:
    try:
        ...
        elif YOUTUBE_COOKIES_RAW.strip():
            payload = YOUTUBE_COOKIES_RAW.encode("utf-8")
        if not payload:
            return None
...
COOKIE_FILE_PATH = prepare_cookie_file()
...
def public_job(job: dict) -> dict:
    ...
    keys = [
        ...
        "source_type", "source_title", "source_duration_seconds",
        "duration_seconds", "output_size_bytes", "trim_mode", "trim_applied",
        "trim_start_seconds", "trim_end_seconds",
        "speed", "bitrate", "roblox_safe",
        "safe_duration_seconds", "worker_version",
...
def download_youtube_audio(job_id: str, url: str) -> Path:
    ...
    set_job(job_id, status="processing", progress=4, message="Preparing smart YouTube import")
    ...
    attempts.extend([
        ...
        ("Android VR", {"extractor_args": {"youtube": {"player_client": ["android_vr"]}}}),
        ...
        attempts.append((
            "Authenticated cookie fallback",
            {
                ...
                "extractor_args": {
                    ...
                    **({"youtubepot-bgutilhttp": {"base_url": [POT_PROVIDER_URL]}} if POT_PROVIDER_URL else {}),
                },
            },
        ))

    last_error = None
    for idx, (label, extra) in enumerate(attempts, start=1):
        for old in job_dir.glob("youtube-source.*"):
            if old.is_file():
                try:
                    old.unlink()
                except OSError:
                    pass
        pct = min(17, 4 + idx * 2)
        set_job(job_id, progress=pct, message=f"YouTube import • trying {label}")
        opts = dict(common)
        opts.update(extra)
        try:
            ...
            if not source or not source.is_file():
                candidates = [
                    p for p in job_dir.glob("youtube-source.*")
            ...
            set_job(job_id, source_path=str(source), message=f"YouTube import succeeded via {label}")
            return source
        except (yt_dlp.utils.DownloadError, RuntimeError) as exc:
            last_error = exc
            continue

    text = re.sub(r"^ERROR:\s*", "", str(last_error or "YouTube import failed")).strip()
    lower = text.lower()
    if "not a bot" in lower or "cookies" in lower or "login" in lower:
        if COOKIE_FILE_PATH:
            friendly = "YouTube blocked this server session even after PO Token and cookie fallback. Please upload the original audio/video file instead."
        else:
            friendly = "YouTube temporarily blocked automated access from this server. PO Token fallbacks were tried. Upload the original audio/video file to continue, or configure the optional Render cookie secret for another fallback."
        raise RuntimeError(friendly) from last_error
    raise RuntimeError("YouTube import unavailable for this video. Please upload the original audio/video file instead.") from last_error
    ...
    try:
        ...
        if trim_mode == "manual":
            ...
            if predicted > HARD_FINAL_SECONDS:
                raise RuntimeError(
                    f"No Trim selected but final audio would be about {predicted:.2f}s. "
                    f"Roblox-safe maximum is {HARD_FINAL_SECONDS:.1f}s."
                )
        else:
            trim_mode = "auto"
            desired_source_len = source_remaining
        ...
        trim_applied = (
            trim_start > 0.001
            or effective_end < source_duration - 0.05
            or safety_clamped
        )
        ...
        set_job(
            ...
            progress=28,
            message=(
                f"Converting source {source_duration:.2f}s • "
                f"final target <= {TARGET_FINAL_SECONDS:.1f}s"
            ),
            source_duration_seconds=round(source_duration, 3),
            trim_mode=trim_mode,
            trim_applied=trim_applied,
            trim_start_seconds=round(trim_start, 3),
            trim_end_seconds=round(effective_end, 3),
        )

        run_ffmpeg([
            "-y", "-hide_banner", "-loglevel", "error",
            "-ss", f"{trim_start:.6f}",
            "-t", f"{clip_source_len:.6f}",
            "-i", str(source),
            "-vn", "-map_metadata", "-1",
        ...
        if final_duration > HARD_FINAL_SECONDS:
            set_job(
                job_id,
                progress=88,
                message=f"Final was {final_duration:.2f}s • enforcing safe 6:58 output",
            )
            run_ffmpeg([
                "-y", "-hide_banner", "-loglevel", "error",
                "-i", str(output),
                "-t", f"{TARGET_FINAL_SECONDS:.3f}",
                "-vn", "-map_metadata", "-1",
                "-ar", "48000", "-ac", "2",
                "-c:a", "libmp3lame", "-b:a", f"{bitrate}k",
                str(safe_output),
        ...
        if final_duration > HARD_FINAL_SECONDS:
            raise RuntimeError(
                f"Final validation failed: {final_duration:.3f}s exceeds "
                f"safe {HARD_FINAL_SECONDS:.1f}s limit"
            )
        if final_size <= 0 or final_size >= MAX_OUTPUT_BYTES:
            raise RuntimeError(
                f"Final validation failed: output size {final_size} bytes is invalid or >= 20 MB"
            )
        ...
        set_job(
            ...
            status="completed",
            progress=100,
            message=f"READY • final {final_duration:.2f}s • {final_size / 1024 / 1024:.2f} MB",
            duration_seconds=round(final_duration, 3),
            output_size_bytes=final_size,
            trim_applied=trim_applied,
            roblox_safe=True,
            safe_duration_seconds=HARD_FINAL_SECONDS,
...
def root():
    return {
        ...
        "service": APP_NAME,
        "version": APP_VERSION,
        "message": "FAZ DJ Audio Worker online",
    }
...
def health():
    cleanup_old_jobs()
    return {
        "ok": bool(FFMPEG and Path(FFMPEG).exists()),
        "service": APP_NAME,
        "version": APP_VERSION,
        "ffmpeg": bool(FFMPEG and Path(FFMPEG).exists()),
        "ffprobe": "ffmpeg-metadata-parser",
        "youtube_import": True,
        "yt_dlp_version": getattr(yt_dlp.version, "__version__", "unknown"),
        "pot_provider_configured": bool(POT_PROVIDER_URL),
        "pot_provider_url": POT_PROVIDER_URL or None,
        "youtube_cookie_fallback_configured": bool(COOKIE_FILE_PATH),
        "youtube_strategy": "pot+mweb -> standard -> embedded -> android_vr -> ios -> cookies(optional)",
        "speed_aware_trim": True,
        "final_ffprobe_validation": True,
        "final_duration_validation": True,
        "target_final_duration_seconds": TARGET_FINAL_SECONDS,
        "safe_final_duration_seconds": HARD_FINAL_SECONDS,
...
async def create_job(
    ...
    trim_mode: str = Form("auto"),
    trim_start: float = Form(0.0),
    trim_end: Optional[str] = Form(None),
    x_faz_worker_key: Optional[str] = Header(None, alias="X-FAZ-WORKER-KEY"),
):
...
def create_youtube_job(
    ...
    job = {
        ...
        "status": "queued",
        "progress": 1,
        "message": "Queued • waiting to fetch YouTube audio",
        "source_type": "youtube",
